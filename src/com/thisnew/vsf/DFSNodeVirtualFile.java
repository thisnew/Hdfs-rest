 ...
 * @author linux_china
 */
public class DFSNodeVirtualFile extends VirtualFile {
    private DFSVirturalFileSystem fileSystem;
    ...
    public DFSNodeVirtualFile(DFSVirturalFileSystem fileSystem, String filePath) {
        ...
        try {
            this.blockInfo = getConnectionWork().forpath(filePath);
            this.isLeaf = blockInfo.getNumChildren() == 0;
    ...
    public VirtualFile getParent() {
        ...
        if (slashIndex == 0) {
            return new DFSNodeVirtualFile(this.fileSystem, "/");
        } else {
            String parentPath = filePath.substring(0, slashIndex);
            return new DFSNodeVirtualFile(this.fileSystem, parentPath);
        }
    }
    //todo
    public VirtualFile[] getChildren() {
        try {
            List<String> children = getConnectionWork().getChildren(filePath);
            if (children != null && !children.isEmpty()) {
                VirtualFile[] files = new VirtualFile[children.size()];
                for (int i = 0; i < children.size(); i++) {
                    String childName = children.get(i);
                    files[i] = new DFSNodeVirtualFile(fileSystem, filePath.endsWith("/") ? filePath + childName : filePath + "/" + childName);
                }
                return files;
            }
        } catch (Exception ignore) {
...
//        if (content == null) {
//            try {
//                this.content = getConnectionWork().getData().storingStatIn(stat).forPath(filePath);
//                if (isSingleFileZip()) {
...
//        }
    ...
    public boolean equals(Object obj) {
        return obj instanceof DFSNodeVirtualFile && ((DFSNodeVirtualFile) obj).getFilePath().equals(filePath);
    }
